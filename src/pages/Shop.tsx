                ...
                <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`flex-shrink-0 lg:w-full text-left p-3 rounded-xl transition-smooth whitespace-nowrap ${
                ...
                {displayedProducts.map((product) => (
                  <div
                    ...
                    className={`card-neon group cursor-pointer ${
                      viewMode === "list" ? "flex gap-3 items-start p-4" : ""
                    }`}
                    onClick={() => handleProductClick(product)}
                  >
                    <div className={`relative overflow-hidden rounded-xl bg-gradient-to-br from-primary/5 to-accent/5 flex-shrink-0 ${
                      viewMode === "list" ? "w-24 h-24 sm:w-32 sm:h-32" : "mb-4"
                    }`}>
                      <img
                        src={product.image}
                        alt={product.name}
                        className={`object-cover group-hover:scale-110 transition-transform duration-500 ${
                          viewMode === "list" ? "w-full h-full" : "w-full h-36 sm:h-48"
                        }`}
                      />
                    ...
                    <div className={`flex flex-col justify-between min-w-0 ${
                      viewMode === "list" ? "flex-1 h-24 sm:h-32" : "space-y-1.5"
                    }`}>
                        ...
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`${
                                viewMode === "list" ? "h-2.5 w-2.5" : "h-2.5 w-2.5"
                              } ${
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className={`font-semibold group-hover:text-primary transition-smooth break-words line-clamp-2 ${
                          viewMode === "list" ? "text-xs" : "text-xs leading-tight"
                        }`}>
                          {product.name}
                        </h3>
                      </div>
                      ...
                      <div className={`${
                        viewMode === "list" ? "space-y-1" : "pt-2 border-t border-border"
                      }`}>
                        <div className="mb-2">
                        ...
                        <Button 
                          className={`w-full btn-neon group ${
                            viewMode === "list" ? "text-xs py-1 px-2 h-6" : "text-xs py-2 h-9"
                          }`}
                          disabled={!product.inStock}
                          onClick={(e) => handleAddToCart(product, e)}
                        >
                          <ShoppingCart className={`group-hover:animate-bounce ${
                            viewMode === "list" ? "h-3 w-3 mr-1" : "h-3 w-3 mr-1"
                          }`} />
